...
author: "Todd W. Pierson (Updated by Nathan W. Byer)"
date: "9 March 2022"
---
...
As noted in previous classes, we have also provided a Linux virtual machine for use on Windows machines. This virtual machine should represent a useful option for running BASH/UNIX-based analyses.
</div>
...
Then, download the data from web campus. Make sure to put it in the working directory that you set above!
...
Next, we'll separately load our R1 and R2 data. This time, we'll use real sequencing data from an custom amplicon sequencing project. These are amplicons for a suite of gene regions expected to be important to climate adaptation in turtles. Keep in mind that these products were all 150 base pairs or less.

```{r, results = "hide", warning = FALSE, message = FALSE}
fq1 <- fastqq(c("data/MC1_S13_L001_R1_001.fastq"))
fq2 <- fastqq(c("data/MC1_S13_L001_R2_001.fastq"))
```
